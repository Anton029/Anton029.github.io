...
const scrollBarWidth = window.innerWidth - document.querySelector('.page_wrapper').offsetWidth + 'px';
let scroll_fix_margin = document.querySelectorAll('.scroll_fix_margin');
let scroll_fix_padding = document.querySelectorAll('.scroll_fix_padding');
let scroll_fix_offset = document.querySelectorAll('.scroll_fix_offset');
...
$(document).ready(function form(){		/*Popup floatup*/
	$('.request_button, .contacts_button').click(function(event){
		...
		if(window.innerWidth > 1200){
			scroll_fix_margin.forEach(element => {
				element.style.marginRight = scrollBarWidth;
				//element.style.marginLeft = element.style.marginLeft - scrollBarWidth;
			});
			scroll_fix_padding.forEach(element => {
...
$(document).ready(function slider(){			/*Slider form*/
	...
	let trackHeight = $('.slide_box');
	let slidesHeight = [$('.slide_1').outerHeight(), $('.slide_2').outerHeight(), $('.slide_3').outerHeight(), $('.slide_4').outerHeight()];
	let gridCollection = $('#grid_1, #grid_2, #grid_3, #grid_4, #grid_5, #grid_6, #grid_7, #grid_8');
	let choiceID;
	let cho;
	let flag_1 = false;
	let flag_2;
	let slideHeight;
	const sliderTabs = $('.slide_step_1, .slide_step_2, .slide_step_3, .slide_step_4');

	sliderTabs.click(function SliderTab(){
		console.log();
	})
	...
	$('.choice_body_wrapper').mouseover(function(){
		if (flag_1 == true){
			return;
		};
	...
	$('.choice_body_wrapper').mouseout(function(){
		if (flag_1 == true){
			return;
		};
		let choiceID = this.id.substring(this.id.length -1);
	...
	$('.contact_wrapper').mouseover(function(){
		if (flag_2 == true){
			return;
		};
		let choiceID = this.id.substring(this.id.length -1);
	...
	$('.contact_wrapper').mouseout(function(){
		if (flag_2 == true){
			return;
		};
		let choiceID = this.id.substring(this.id.length -1);
...
$(document).ready(function kamaz(){
	let kamazPosition = $('.truck_kamaz').offset().top;
	const winHeight = window.innerHeight;
